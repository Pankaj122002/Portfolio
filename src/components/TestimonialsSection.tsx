...
function Card({ quote, author, company }: typeof testimonials[0]) {
  return (
    <div className="flex-shrink-0 w-80 md:w-96 px-3">
      <div className="glass-premium rounded-2xl p-7 transition-all duration-300 hover:border-indigo/20">
        <div className="flex gap-1 mb-5">
          {Array.from({ length: 5 }).map((_, i) => (
            <svg key={i} className="w-4 h-4" fill="#F59E0B" viewBox="0 0 20 20" style={{ filter: 'drop-shadow(0 0 4px rgba(245,158,11,0.4))' }}>
        ...
        <div className="flex items-center gap-3">
          <div
            className="w-10 h-10 rounded-full flex items-center justify-center text-white text-sm font-display font-bold"
            style={{ background: 'linear-gradient(135deg, #6366F1, #06B6D4)', boxShadow: '0 0 15px rgba(99,102,241,0.3)' }}
          >
            {author.charAt(0)}
          </div>
          <div>
...
export default function TestimonialsSection() {
  ...
  return (
    <section className="relative py-20 overflow-hidden" style={{ zIndex: 10 }}>
      ...
      <div className="text-center mb-10 gsap-text-up">
