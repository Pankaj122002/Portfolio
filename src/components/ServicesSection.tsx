...
const services = [
  { icon: Globe, title: 'Business & Restaurant Websites', desc: 'Conversion-focused, fast-loading sites that turn visitors into customers.', color: '#4F46E5' },
  ...
  { icon: LayoutDashboard, title: 'Admin Dashboards & Data Viz', desc: 'Real-time dashboards with interactive charts and analytics.', color: '#7C3AED' },
  ...
  { icon: Container, title: 'Docker & Deployment Setup', desc: 'Containerization, CI/CD pipelines, and cloud deployment automation.', color: '#8B5CF6' },
];
...
function ServiceCard({ icon: Icon, title, desc, color, index }: typeof services[0] & { index: number }) {
  ...
  return (
    <div
      ref={ref}
      className="glass rounded-2xl p-4 group transition-all duration-300"
      style={{
        opacity: visible ? 1 : 0,
        translate: visible ? 'none' : '0 30px',
        transition: `opacity 0.6s ease ${index * 0.06}s, translate 0.6s ease ${index * 0.06}s, box-shadow 0.3s ease`,
      }}
      onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.boxShadow = `0 0 40px ${color}20`; }}
...
export default function ServicesSection() {
  return (
    <section id="services" className="relative py-16 px-4 md:px-8" style={{ zIndex: 10 }}>
